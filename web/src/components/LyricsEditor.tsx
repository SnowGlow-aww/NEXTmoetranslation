...
import {
  ...
  getLyrics, getProjectionStatus, issueLyricsCollabTicket, previewLyricsSource, publishLyrics, saveLyrics,
...
export const LyricsEditor = forwardRef<LyricsEditorHandle, LyricsEditorProps>(function LyricsEditor({ role, reloadGeneration, writeLocked: producerWriteLocked = false, onDirtyChange }, ref) {
  ...
  const performChooseMusic = useCallback(async (item: CatalogMusicItem) => {
    ...
    try {
      ...
      const editableRendition = isRenditionLyricsDocument(authoritative) ? authoritative as RenditionLyricsDocument : null;
  ...
  }, [requestIsCurrent, startCollaboration, stopCollaboration]);
  ...
  const discard = () => {
    ...
    setSourceSearchCompleted(false);
    setError(null);
    if (selectedMusicIDRef.current === lyrics.musicId) startCollaboration(lyrics.musicId);
  };
  ...
  const continuePendingTransition = async (saveFirst: boolean) => {
    ...
    } else if (pending.kind === "choose" && dirty) {
      lyricsLoadSequence.current++;
      if (sourceImportTokenRef.current && baseline) {
        document = JSON.parse(baseline) as SongLyricsDocument;
        setLyrics(document);
      } else {
        if (hasLocalUndo) collaborationRef.current?.discardLocalChanges();
        document = collaborationRef.current?.getSnapshot().document ?? document;
      }
      sourceImportTokenRef.current = "";
      setSourcePreview(null);
      setSourcePreviewCandidate(null);
      ...
      <Modal open={confirmConflictReload && error?.code === "revision_conflict" && error.current != null} onClose={() => setConfirmConflictReload(false)} title="载入服务器版本" maxWidth={500}>
        ...
        <div className="dirty-guard-actions">
          <button className="btn btn-secondary" onClick={() => {
            ...
            setError(null);
            setConfirmConflictReload(false);
            startCollaboration(error.current.musicId);
          }}>确认载入服务器版本</button>
